...
function Card() {
    ...
    useEffect(() => {
      async function fetchData() {
        if (resID && resID.studentId) {
          const response = await axios.get(`http://localhost:8080/student/get-by-student-id/${resID.studentId}`);
  ...
  {tableData.map((item, index) => (
    <a href="#" className="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 flex flex-col justify-between">
      <div>
        <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{item.subject}</h5>
        <p className="font-normal text-gray-700 dark:text-gray-400">{item.noOfCredits}</p>
      </div>
      <button className="px-4 py-2 bg-gray-900 text-white rounded-lg mt-4 hover:bg-gray-800">Enroll Now</button>
    </a>
  ))}
</div>
 </div>
